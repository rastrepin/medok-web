...
function ObBio({ id, onInfo }: { id: string; onInfo: (k: string) => void }) {
  ...
  if (id === 'kelman') {
    return (
      <p style={{ fontSize: 13, color: 'var(--g500)', lineHeight: 1.65, marginBottom: 16, textAlign: 'center' }}>
        Фокус на пренатальній діагностиці. Навчання за програмою FMF (Fetal Medicine Foundation) — міжнародний стандарт скринінгу. Детально пояснює кожен результат і план дій.
      </p>
    );
  }
  if (id === 'trofimchuk') {
    return (
      <p style={{ fontSize: 13, color: 'var(--g500)', lineHeight: 1.65, marginBottom: 16, textAlign: 'center' }}>
        Досвід у перинатальному центрі та пологовому будинку Інномед. Веде вагітність від постановки на облік до пологів — включно з ускладненнями.
      </p>
    );
  }
  return null;
